...
* @Last Modified time: 2017-06-06 11:04:21
*/
		...
		return new Promise((success, fail) => {
			...
			rstream   // reads from inputFilePath
				...
				.on('error', (err) => {
					...
					return fail (errs.merge(err, new Error('Unable to encrypt')));
				})
				.on('finish', () => {  // finished
	...
	decryptCBC() {
		...
		return new Promise((success, fail) => {
			...
			rstream   			// reads from filepath
				...
				.on('error', (err) => {
					// console.log('Oops! below error occured : \n', err);
					return fail (errs.merge(err, new Error('Unable to decrypt. Bad Key')));
				})
				.on('data', (buffer) => {  // finished
